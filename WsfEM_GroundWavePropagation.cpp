...
// WsfEM_GroundWavePropagation.cpp 中文注释整理版本
// 功能：实现ITU-R地波传播模型（GRWAVE）的数值传播计算
...
// =================================================================================================
//1.	默认构造函数:
//•	初始化了多个成员变量，包括相对介电常数、导电率、对流层折射率等。
//•	使用了 std::complex<double> 来表示复数，用于处理波数、阻抗等复杂计算。
//•	初始化了多个数组（如 mImpedanceFunction 和 mReflectionFunction），这些数组用于存储传播计算中的中间结果。
//•	关键点 :
//•	数组初始化采用循环，确保所有元素的初始值为零。
//•	物理参数的默认值基于地波传播模型的典型设置。
// - mRelativePermittivity: 相对介电常数，影响波在地表传播时的折射与反射
// - mConductivity: 地面导电率，影响地面能量损耗
// - mTroposphereRefractivity: 对流层折射率，反映电磁波在大气中的传播弯曲程度
// - mTroposphereHeightScale: 对流层高度尺度（km），决定折射率随高度变化速率
// - mDel: 折射率增量参数，后续计算中用于修正曲率因子
// - mMinDistance / mDistanceInterval: 最小传播计算距离与间隔（km）
// - mWavenumber: 波数，频率相关的基本传播参数（单位：1/km）
// - mImpedanceFunction, mReflectionFunction 等数组：用于保存模态阻抗、反射、激励等复数中间结果
// - mP, mDp: 波动方程中的复数传播常数及其导数（数值积分用）
WsfEM_GroundWavePropagation::WsfEM_GroundWavePropagation()
    : mRelativePermittivity(70.0),                // 地表相对介电常数（典型湿土）
    mConductivity(5.0),                         // 地面导电率（mS/m）
    mTroposphereRefractivity(315.0),            // 对流层折射率 N 值
    mTroposphereHeightScale(7.35),              // 对流层高度尺度（单位：km）
    mDel(0.0),
    mMinDistance(10.0),                         // 最小传播距离（km）
    mDistanceInterval(10.0),                    // 传播计算间隔（km）
    mWavenumber(0.0),
    mWavenumberSquared(0.0),
    mWavenumberImaginary(0.0, 0.0),             // 复数波数
    mP(0.0),                                    // 传播函数 P(h)
    mIntegrationPhase(-1),
    ...
    mReflectionNF(0),
    mXmtrAlt(1.0),                              // 发射机高度（米）
    mRcvrAlt(1.0)                               // 接收机高度（米）
{
    // 初始化所有模态相关的复数变量（共支持9个模式）
   for (size_t i = 0; i < 9; ++i)
   {
       mImpedanceFunction[i] = std::complex<double>(0.0, 0.0);           // 阻抗函数
       mReflectionFunction[i] = std::complex<double>(0.0, 0.0);          // 反射函数
       mReflectionDerivativeFunction[i] = std::complex<double>(0.0, 0.0);// 反射函数导数
       mP0[i] = std::complex<double>(0.0, 0.0);                          // 每个模式的传播常数初始值
       mTurningPoint[i] = std::complex<double>(0.0, 0.0);               // 转折点（P(h)=0 的高度）
       mHeightChange[i] = 0.0;                                          // 高度增益函数的分段阈值
   }

   for (size_t i = 0; i < 4; ++i)
   {
      mDp[i] = std::complex<double>(0.0, 0.0);  // P(h)的一阶到三阶导数
   }
}
...
//static
//•	ObjectFactory 方法根据输入的类型名称创建 WsfEM_GroundWavePropagation 对象。
WsfEM_Propagation* WsfEM_GroundWavePropagation::ObjectFactory(const std::string& aTypeName)
{
   ...
   if ((aTypeName == "WSF_GROUND_WAVE_PROPAGATION") ||
       (aTypeName == "ground_wave_propagation") ||
...
// =================================================================================================
//virtual
//•	处理输入命令，允许用户动态设置传播参数（如相对介电常数、导电率等）。
//•	使用了 UtInput 类来读取和验证输入值。
//•	实现细节 :
//•	根据输入命令(command) 调用相应的处理逻辑。
//•	使用 UtInput 类读取和验证输入值。
//•	对某些参数（如长度）进行单位转换（如从米转换为千米）。
//•	关键点 :
//•	确保输入值大于零，避免无效参数。
//•	支持多种命令，灵活性高。
...
//•	主要步骤：
//1.	获取发射机和接收机的频率。
//2.	设置指数大气模型参数（调用 SetupExponentialAtmosphere）。
//3.	计算波数和地球的折射率平方。
//4.	根据极化类型调整表面阻抗。
//5.	分别计算从发射机到目标和从目标到接收机的传播因子。
//6.	返回总传播因子（出站传播因子 × 入站传播因子）。
double WsfEM_GroundWavePropagation::ComputePropagationFactor(WsfEM_Interaction& aInteraction,
                                                             WsfEnvironment&    aEnvironment)
{
    // 获取频率，优先使用接收机频率（如果非被动接收机）
   ...
   complex<double> dummyComplex(0.0, mWavenumber);
   mWavenumberImaginary = dummyComplex;

   // 计算地球介质复折射率平方（包括相对介电常数和导电率）
   complex<double> nSquared(mRelativePermittivity, -1.8e4 * mConductivity / (frequency * 1.e-6));

   // 表面阻抗（归一化到自由空间）
   complex<double> surfaceImpedance = std::sqrt(nSquared - 1.0);

   // 若为垂直极化，需调整阻抗和折射率
   if (aInteraction.GetTransmitter()->GetPolarization() != WsfEM_Types::cPOL_HORIZONTAL)
   {
      surfaceImpedance /= nSquared;
      ModifyValuesForVerticalPol(scale,
                                 d1p0,
                                 surfaceImpedance,
                                 d1p0,
                                 surfaceImpedance);
   ...
   // 优先尝试远场传播模型
   bool farFieldValid = FarFieldTransmissionLoss(aInteraction,
                                                 scale,
                                                 d1p0,
                                                 surfaceImpedance,
                                                 propagationFactorOutbound);
   // 若远场传播不适用，则退回到几何光学模型
   if (! farFieldValid)
      GeometricalOptics(aInteraction,
                        scale,
                        d1p0,
                        surfaceImpedance,
                        propagationFactorOutbound);
   ...
   // 同样优先使用远场传播
   farFieldValid = FarFieldTransmissionLoss(aInteraction,
                                            scale,
                                            d1p0,
                                            surfaceImpedance,
                                            propagationFactorInbound);
   if (! farFieldValid)
      GeometricalOptics(aInteraction,
                        scale,
                        d1p0,
...
//!virtual protected
//!设置指数大气模型的相关参数，如有效地球半径因子和折射率增量
//! •	实现细节:
//•	计算 mDel（折射率增量）和 aScale（地球半径与对流层高度的比值）。
//•	计算有效地球半径因子 k0 和相关参数 aD1P0。
//•	参数计算基于 ITU - R 指数大气模型。
                                                             ...
                                                             std::complex<double>& aSurfaceImpedanceV)
{
   ...
   aSurfaceImpedanceV = aSurfaceImpedance + mDel * aScale /
                        (6.37e6 * (1.0 + mDel) * mWavenumberImaginary);
                                                           ...
                                                           double&               aPropagationFactor)
{
   ...
   // fid(m) m=0,8
   // 模态相关变量定义
   complex<double> fid[9];   // 模态激励因子
   complex<double> p0Sub[9];    // 初始传播常数备份
   complex<double> sm1[9];  // 修正波数项
   complex<double> exc[9];  // 模态激励项对数
   // 计算模态激励项和传播常数
   for (unsigned int i = 0; i < 9; ++i)
   {
      Eigen(this,
            ...
            fid);
      p0Sub[i] = mP0[i];
      sm1[i] = mWavenumberImaginary * (mDel - mP0[i]) /
               (std::sqrt(1.0 + mDel - mP0[i]) + 1.0);
   ...
   for (unsigned int i = 0; i < 9; ++i)
   {
      ...
      complex<double> reflectionCoefficient = 1.0 - 2.0 * aImpedance /
                                              (aImpedance + std::sqrt(mP0[i]));
   ...
   // 计算残差级数展开项并判断收敛性
   double zeroLossFieldStrength =
      UtMath::DB_ToLinear(BasicTransmissionLoss(aInteraction));
   ...
   {
      index = i;
      complex<double> termLarge = -1.e3 * range * sm1[i] +
                                  heightGainRcvr[i] + heightGainXmtr[i] - exc[i] + a;
...
//!virtual protected
//! •	使用几何光学方法计算传播因子，适用于近场或传播路径复杂的情况。
//! •	实现细节:
//•	计算传播路径的几何参数（如地面距离、转折点等）。
//•	使用几何光学模型计算传播损耗。
//•	关键点 :
//•	适用于远场模型失效的情况。
//•	涉及高度和距离的几何关系计算。
                                                    ...
                                                    double&               aPropagationFactor)
{
   ...
   double zeroLossFieldStrength =
      UtMath::DB_ToLinear(BasicTransmissionLoss(aInteraction));
   complex<double> heightLowC(min(mRcvrAlt, mXmtrAlt), 0.0);
   complex<double> heightHighC(max(mRcvrAlt, mXmtrAlt), 0.0);
   ...
   if ((mRcvrAlt > hn) || (mXmtrAlt > hn))
   {
      ...
      mMinDistance = max(distanceMin, mMinDistance);
      ...
      while (ifin == 0)
      {
         ...
         edexp += 0.5 * log(5.625e12 * mWavenumber * g[m1] * g[m1] / (d * fdd[m1]))
                  + f[m1];
         ...
         erexp += 0.5 * log(5.625e12 * mWavenumber / (d * fdd[2])) +
                  f[2] + r;
   ...
   {
      aPropagationFactor = FlatEarthPropagation(aInteraction,
                                                aScale,
                                                aImpedance,
                                                heightLowC,
                                                heightHighC);
                                        ...
                                        std::complex<double>         aFid[9])
{
   ...
   for (unsigned int iter = 0; iter < 9; ++iter)
   {
      ...
      factor1 *= factor;
      complex<double> ddf = r03 + aImpedance * (aImpedance + 3.0 * factor) /
                            (2.0 * factor1 * factor1 * factor1);
      dp0 = -f / df;
   ...
   aFid[aMode] = mWavenumberImaginary / (1.0 + r0[0]) *
                 (0.5 * (1.0 - r0[0]) / factor - 2.0 * factor * r0[1] / (1.0 + r0[0]));
}
...
double WsfEM_GroundWavePropagation::ComputeGroundDistance(WsfEM_Interaction& aInteraction)
{
   ...
   UtSphericalEarth::GreatCircleHeadingAndDistance(xmtrLat,
                                                   xmtrLon,
                                                   tgtLat,
                                                   tgtLon,
                                                   dummyAngle,
                                                   ...
                                                   double                       aD1P0)
{
   ...
   {
      -2.338107, -4.087949, -5.520560, -6.786708, -7.944134,
      -9.022651, -10.04017, -11.00852, -11.93602, -12.82878
   };
   // asd are the first 10 roots of dai(z) / dz = 0
   static const double cASD[10] =
   {
      -1.018793, -3.248198, -4.820099, -6.163307, -7.372177,
      -8.488487, -9.535449, -10.52766, -11.47506, -12.38479
   };
...
//!virtual protected
complex<double> WsfEM_GroundWavePropagation::PhaseIntegral(int                   aCode,
                                                           ...
                                                           double                aScale)
{
   ...
   static const double cY1[16] =
   {
      0.005143979, 0.02690510, 0.06525346, 0.1188529,
      0.1858245, 0.2638194, 0.3501023, 0.4416466,
      0.5352415, 0.6276043, 0.7154952, 0.7958316,
      0.8657955, 0.9229332, 0.9652404, 0.9912332
   };
   static const double cW1[16] =
   {
      0.01314449, 0.02981894, 0.04470995, 0.05692274,
      0.06578789, 0.07089086, 0.07210076, 0.06957807,
      0.06375982, 0.05532300, 0.04512950, 0.03415651,
      0.02341822, 0.01388466, 0.006404388, 0.00163687
   };
   static const double cY2[16] =
   {
      0.00546479, 0.02856995, 0.06923379, 0.1259506,
      0.1966084, 0.2785747, 0.3687955, 0.4639093,
      0.5603723, 0.6545902, 0.7430525, 0.8224631,
      0.8898632, 0.9427416, 0.9791279, 0.9976664
   };
   static const double cW2[16] =
   {
      0.01403722, 0.03254879, 0.05078413, 0.06854773,
      0.08567180, 0.1019961, 0.1173682, 0.1316444,
      0.1446916, 0.1563878, 0.1666238, 0.1753042,
      0.1823478, 0.1876888, 0.1912774, 0.1930802
   };
   ...
   for (unsigned int i = 0; i < 16; ++i)
   {
      ...
      else
      {
         ...
         complex<double> cOne(1.0, -1.0);
         complex<double> temp(0.0, -2.0);
         phaseIntegral += cW2[i] * mDp[1] / (mDp[0] * mDp[0]) * cOne *
                          sqrt((1.0 - cY2[i]) / (temp * mP));
      }
   }
   ...
   else
   {
      complex<double> cOne(1.0, -1.0);
      complex<double> temp(0.0, -2.0);
      phaseIntegral = 0.5 * mWavenumberImaginary *
                      (cOne / (mDp[0] * sqrt(temp * mP)) + f * phaseIntegral);
   }
...
{
   complex<double> cZero(0.0, 0.0);
   complex<double> z[2] = { cZero, cZero };
   complex<double> y[2] = { cZero, cZero };
   ...
   for (unsigned int j = 0; j < 2; ++j)
   {
      ...
      if (x2 > 0.25)
      {
         if (std::real(x) > -50.0) { z[j] = std::exp(x); }
         y[j] = z[j] - 1.0;
      }
      else
      {
         complex<double> temp(1.0, 0.0);
         y[j] = temp;
         if (x2 > 0.0001)
            y[j] = 1.0 + x * 0.2 * (1.0 + x / 6.0 *
                                    (1.0 + x / 7.0 * (1.0 + x / 8.0)));
         y[j] = x * (1.0 + x * 0.5 * (1.0 + x / 3.0 *
                                      (1.0 + x * y[j] * 0.25)));
         z[j] = 1.0 + y[j];
      }
   }

   complex<double> delu = mDel * z[1];
   mP = y[0] * (2.0 + y[0]) * (1.0 + delu) + mDel * y[1] + mP0[aMode];

   if (aN == 0) { return; }

   complex<double> t = aScale * z[0];
   complex<double> z2 = z[0] * z[0];
   ...
   // third derivative
   mDp[2] = 3.09594e-20 * z2 * (1.0 + delu * (1.0 - t *
                                              (4.75 - t * (4.5 - t))));
   if (aN == 3) { return; }

   // fourth derivative
   mDp[3] = 9.71171e-27 * z2 * (1.0 + delu * (1.0 - t *
                                              (8.125 - t * (13.75 - t * (7.0 - t)))));
}
   ...
   if (std::real(mP0[aMode]) >= 1.e-3)
   {
      ...
      if (std::real(mP0[aMode]) >= 1.e-2)
      {
         ...
         for (unsigned int i = 0; i < 9; ++i)
         {
            ...
            double d = 0.0;
            if (c > -20.0) { d = exp(c); }
            b = (UtMath::cPI + aScale * std::imag(mP0[aMode])) /
                (aScale + d / mDel);
      ...
      else
      {
         ...
         complex<double> cTemp1(1.0, -1.0);
         complex<double> cTemp2(0.0, 1.0);
         mTurningPoint[aMode] = a * (b + cTemp1 *
                                     sqrt(cTemp2 * (-1.0 - b + c - aScale * mP0[aMode])));
      }
   }
   else
   {
                                            ...
                                            int              aMStep)
{
   ...
   aMStep = 0;
   double step = min(0.5, aStepIn);

   // select smallest value from list
   bool finished = false;
   if (step >= 0.5) { finished = true; }
   complex<double> dx = aXf - aXi;
   aPtr->mIntegrationStep = step * dx;
   double tau = 0.0;
   complex<double> x1 = aXi;

   complex<double>* y1 = new complex<double>[aN];
   for (int j = 0; j < aN; ++j)
   {
   ...
   while (true)
   {
      ...
      if (aMStep < 1000)
      {
         ...
         for (int j = 0; j < aN; ++j)
         {
            ...
            complex<double> tError = (aYf[j] - y3s[j]) / 15.0;

            aYf[j] += tError;

            // equation 6.14.15 of Hildebrand
            complex<double> ec = y1[j] + y2[j] + aYf[j];
         ...
         else
         {
            ...
            tau += 2.0 * step;
            if (err2 == 0.0) { err2 = 1.0e-14; }
            step = max(0.0002, step * pow((aError2 / err2), 0.1));
            if ((tau + 2.0 * step) >= 1.0) { finished = true; }
            if (finished) { step = 0.5 * (1.0 - tau); }
            aPtr->mIntegrationStep = step * dx;
            doubleLength = true;
         }
      }
                                                  ...
                                                  int                   aN)
{
   ...
   complex<double> ak[4] =
   {
      complexZero, complexZero, complexZero, complexZero
   };
...
void WsfEM_GroundWavePropagation::Height(int                   aMode,
                                         ...
                                         double                aHeightB)
{
   complex<double> hac(aHeightA, 0.0);
   complex<double> hbc(aHeightB, 0.0);
   double hc = std::real(mTurningPoint[aMode]);
   complex<double> hcc(hc, 0.0);
   complex<double> hchc(mHeightChange[aMode], 0.0);

   complex<double> fii[2] = { aReflection, aHeightGainFunction };
   complex<double> fif[2];
   fif[0] = aReflection;
   complex<double> htgi = aHeightGainFunction;
   ...
   if (aHeightA < hc)
   {
      if (aHeightB > hc)
      {
         ...
         PFunctions(0,
                    ...
                    aScale,
                    hcc);
         fii[0] = mWavenumberImaginary * sqrt(mP) * (1.0 - fif[0]) /
                  (fif[0] + 1.0);
         fii[1] = fif[1];
...
//!static
complex<double> WsfEM_GroundWavePropagation::Hr(WsfEM_GroundWavePropagation* aPtr,
                                                int                          aCode,
                                                int                          aMode,
                                                double                       aScale,
                                                std::complex<double>&        aHeight,
...
//! used as a stub to call WaveReflection with appropriate values
//!static
complex<double> WsfEM_GroundWavePropagation::HTau(WsfEM_GroundWavePropagation* aPtr,
                                                  int                          aCode,
                                                  int                          aMode,
                                                  double                       aScale,
                                                  std::complex<double>&        aHeight,
...
//! used as a stub to call WaveReflection with appropriate values
//!static
complex<double> WsfEM_GroundWavePropagation::HrTau(WsfEM_GroundWavePropagation* aPtr,
                                                   int                          aCode,
                                                   int                          aMode,
                                                   double                       aScale,
                                                   std::complex<double>&        aHeight,
...
// =================================================================================================
//!virtual protected
complex<double> WsfEM_GroundWavePropagation::WaveReflection(int                   aFunctionCode,
                                                            int                   aCode,
                                                            int                   aMode,
                                                            double                aScale,
                                                            std::complex<double>& aHeight,
                                                            std::complex<double>* aFunction)
{
   ...
   if ((mReflectionCounter == 1) && (aCode == 0))
   {
      ...
      for (unsigned int n = 0; n < 4; n += loopInterval)
      {
         PFunctions(1,
                    aMode,
                    aScale,
                    (aHeight + (0.5 * (n + 1) * mIntegrationStep)));
   ...
   {
      if (aFunctionCode == 1)
         value = -mWavenumberImaginary *
                 (aFunction[0] / mReflectionFunction[mReflectionNF - 1] + 2.0 * mReflectionFunction[mReflectionNF - 1] * aFunction[1]) -
                 mReflectionDerivativeFunction[mReflectionNF - 1] * (0.25 * (1.0 - aFunction[0] * aFunction[0]) /
                                                                     (mReflectionFunction[mReflectionNF - 1] * mReflectionFunction[mReflectionNF - 1]) + 0.5 * aFunction[0] * aFunction[1]);

      if (aFunctionCode == 3)
         value = mWavenumberImaginary * mReflectionFunction[mReflectionNF - 1] * (1.0 - aFunction[0]) /
                 (1.0 + aFunction[0]);
   }
...
//!static
complex<double> WsfEM_GroundWavePropagation::Hfi(WsfEM_GroundWavePropagation* aPtr,
                                                 int                          aCode,
                                                 int                          aMode,
                                                 double                       aScale,
                                                 std::complex<double>&        aHeight,
                                                 std::complex<double>*        aFunction)
{
   return aPtr->WaveImpedanceFunction(1,
                                      aCode,
                                      aMode,
                                      aScale,
                                      aHeight,
...
//! used as a stub to call WaveImpedanceFunction with appropriate values
//!static
complex<double> WsfEM_GroundWavePropagation::HfiTau(WsfEM_GroundWavePropagation* aPtr,
                                                    int                          aCode,
                                                    int                          aMode,
                                                    double                       aScale,
                                                    std::complex<double>&        aHeight,
                                                    std::complex<double>*        aFunction)
{
   return aPtr->WaveImpedanceFunction(2,
                                      aCode,
                                      aMode,
                                      aScale,
                                      aHeight,
...
//!virtual protected
complex<double> WsfEM_GroundWavePropagation::WaveImpedanceFunction(int                   aFunctionCode,
                                                                   int                   aCode,
                                                                   int                   aMode,
                                                                   double                aScale,
                                                                   std::complex<double>& aHeight,
                                                                   std::complex<double>* aFunction)
{
   ...
   if ((mImpedanceCounter == 1) && (aCode == 0))
   {
      ...
      for (unsigned int count = 0; count < 4; count += loopStep)
      {
         PFunctions(0,
                    aMode,
                    aScale,
                    aHeight + 0.5 * (count + 1) * mIntegrationStep);
   ...
   if (aCode == 0)
   {
      ...
      value = -mWavenumberSquared * mImpedanceFunction[mReflectionNF - 1] -
                                                         ...
                                                         std::complex<double>  aHeightHighC)
{
   ...
   while ((! convergence) && (distance <= range))
   {
      ...
      if (std::real(aHeightHighC) > 0.0) { er = exp(taur * taur); }
...
{
   // define static data
   static const double cY3[16] =
   {
      -0.09501251, 0.09501251, -0.28160355, 0.28160355,
      -0.45801678, 0.45801678, -0.61787624, 0.61787624,
      -0.75540440, 0.75540440, -0.86563120, 0.86563120,
      -0.94457502, 0.94457502, -0.98940093, 0.98940093
   };

   static const double cW3[16] =
   {
      0.18945061, 0.18945061, 0.18260342, 0.18260342,
      0.16915652, 0.16915652, 0.14959599, 0.14959599,
      0.12462897, 0.12462897, 0.09515851, 0.09515851,
      0.06225352, 0.06225352, 0.02715246, 0.02715246
   };
   ...
   if (! lh)
   {
      ...
      for (unsigned l = 0; l < lMax; ++l)
      {
         ...
         alp[6][l] += -0.5 * ai[1][l] * (0.5 * alp[3][l] + 0.125 * ai[1][l]) +
                      0.5 * ai[0][l] * ai[2][l];
      }
...
{
   complex<double> a[34];
   complex<double>* b = &a[17];

   // compute j[0] & j[1]
   complex<double> dummyC(1.0, 0.0);
   aJ[0] = dummyC;
   complex<double> dummyC2(0.0, 1.772454);
   ...
   if (std::norm(1.0 + b[aMaxN - 1] / a[aMaxN - 1]) < 1.e-5)
   {
      ...
      a[33] = complexZero;
      a[32] = dummyC;
      complex<double> factor = 0.5 / (aTau * aTau);
                                                ...
                                                std::complex<double>  aJrp[4][7])
{
   ...
   if (lt)
   {
      ...
      {
         factor *= dt;
         complex<double> term = factor * j[n];
         aJrp[1][5] += term;
   ...
   {
      aJrp[2][m] = -2.0 * aJrp[1][m - 1] - 2.0 * aTauR * aJrp[1][m] -
                   static_cast<double>(m) * aJrp[1][m + 1];
   }
...
//!virtual protected
complex<double> WsfEM_GroundWavePropagation::WFunction(std::complex<double> aZ)
{
   ...
   if ((0.238 * fabs(zr) + 0.269 * fabs(zi) > 1.0) ||
       (0.3225 * fabs(zr) - 0.37 * fabs(zi) > 1.0) ||
       (0.09 * fabs(zr) + 0.4 * fabs(zi) > 1.0))
   {
      ...
      for (unsigned int m = 0; m < 40; ++m)
      {
         ...
         {
            returnValue = sum + ((m + 1) + 0.5) * term * factor *
                          (1.0 + zSquared * factor * factor);
         }
         else
         {
            returnValue = sum + term * (-1.0 + zSquared * dummyC /
                                        sqrt(2.0 * (m + 1) + 1.0) + (zSquared - static_cast<double>(m + 1) - 0.5) /
                                        static_cast<double>(m + 1 + 0.5));
         }
      ...
      for (unsigned int m = 0; m < 40; ++m)
      {
         if (type)
         {
            ...
            if ((m + 1) >= cz + 2)
            {
               ...
               returnValue += (factor * (1.0 - factor * factor / (m + 1 + 1.0)) -
                               1.0 / ((m + 1) + 1.0)) * term;
            }
         }
         else
...
//!virtual protected
complex<double> WsfEM_GroundWavePropagation::PhaseIntegralAB(int                   aCode,
                                                             int                   aMode,
                                                             double                aScale,
                                                             ...
                                                             std::complex<double>& aHeightB)
{
   ...
   complex<double> fp = 0.5 * (aHeightB + aHeightA);
   complex<double> fm = fp - aHeightA;
   complex<double> returnValue(0.0, 0.0);
   ...
   for (unsigned int i = 0; i < 16; ++i)
   {
      ...
      complex<double> dumC(0.0, -0.5);
      complex<double> factor = cW3[i] * sqrt(dumC * mP);
      if (aCode >= 2)
      { factor /= mP; }
      if (aCode == 3)
      { factor /= mP; }
   ...
   complex<double> complexOne(1.0, 1.0);
...
//!virtual protected
complex<double> WsfEM_GroundWavePropagation::PhaseIntegralCombinations(int                  aCode,
                                                                       int                  aMode,
                                                                       double               aScale,
                                                                       ...
                                                                       std::complex<double> aHeightHigh)
{
   ...
   if (aMode == 0)
   {
      if (std::real(aHeightLow - mTurningPoint[aMode]) < max(10.0, std::real(aHeightHigh - aHeightLow)))
      {
         if (std::real(aHeightLow - mTurningPoint[aMode]) < 0.0)
         {
            returnValue = -PhaseIntegral(aCode, aHeightLow, aMode, aScale) -
                          PhaseIntegral(aCode, aHeightHigh, aMode, aScale);
         }
         else
         {
            returnValue = PhaseIntegral(aCode, aHeightLow, aMode, aScale) -
   ...
   {
      complex<double> complexZero(0.0, 0.0);
      bool a = (std::real(-mTurningPoint[aMode]) < max(10.0, std::real(aHeightLow)));
      bool b = (std::real(-mTurningPoint[aMode]) < max(10.0, std::real(aHeightHigh)));
      if (a)
         returnValue = 2.0 * PhaseIntegral(aCode, complexZero, aMode, aScale) -
                       PhaseIntegral(aMode, aHeightLow, aMode, aScale) -
                                            ...
                                            std::complex<double>  aG[9])
{
   ...
   complex<double> factor = 500.0 * aDistance * mWavenumberImaginary /
                            (1.0 + mDel);
   TurningPoint(aScale, aMode);
   for (unsigned int iter = 0; iter < 9; ++iter)
   {
      complex<double> dp0 = (factor + PhaseIntegralCombinations(2,
                                                                aMode,
                                                                aScale,
                                                                aHeightLow,
                                                                aHeightHigh)) /
                            PhaseIntegralCombinations(3,
                                                      aMode,
                                                      aScale,
                                                      aHeightLow,
                                                      aHeightHigh);
   ...
   aF[aMode] = factor * mP0[aMode] + PhaseIntegralCombinations(1,
                                                               aMode,
                                                               aScale,
                                                               aHeightLow,
                                                               aHeightHigh);
   aFdd[aMode] = PhaseIntegralCombinations(3,
                                           aMode,
                                           aScale,
                                           aHeightLow,
                                           aHeightHigh);
