...
namespace
{
string sLastImplicitBeamCommand;
}
...
// =================================================================================================
//virtual按模式和索引获取接收器。
WsfEM_Rcvr& WsfRadarSensor::GetEM_Rcvr(size_t aModeIndex,
                                       size_t aIndex) const
{
   if (aModeIndex < mRadarModeList.size())
   {
...
// =================================================================================================
//virtual返回当前激活的发射器数量。可根据模式或启用状态获取。
...
// =================================================================================================
//virtual获取发射器对象引用，优先当前启用列表。
WsfEM_Xmtr& WsfRadarSensor::GetEM_Xmtr(size_t aModeIndex,
                                       size_t aIndex) const
{
   if (aModeIndex < mRadarModeList.size())
   {
...
//! GetEM_Xmtr methods.
//virtual内部辅助函数：根据当前激活模式更新发射器/接收器列表，用于 GetEM_*() 方法
...
// =================================================================================================
//virtual初始化各个雷达模式，检查是否具备发射/接收能力：
bool WsfRadarSensor::Initialize(double aSimTime)
{
   ...
   {
      SetClass(cSEMI_ACTIVE | cRADIO);      // 若不可发射，则设为“半主动雷达”；
   }
   return ok;
}

// =================================================================================================
//virtual注册平台，只在模式支持接收时调用父类注册
void WsfRadarSensor::PlatformAdded(double       aSimTime,
...
void WsfRadarSensor::Update(double aSimTime)
{
   ...
   {
      WsfArticulatedPart::UpdatePosition(aSimTime);   // Ensure my position is current
      PerformScheduledDetections(aSimTime);           // 执行探测
   }
}
...
//关键函数：调度系统选择目标并尝试探测：
//获取调度器、追踪器、模式
//调用 RadarMode::AttemptToDetect() 进行探测尝试。
      ...
      else if (TransientCueActive() && (targetIndex == 0))
      {
         ...
         if (modePtr->WithinDetectionRange(aSimTime, mTempGeometryPtr))
         {
            for (unsigned int xmtrIndex = 0; xmtrIndex < GetEM_XmtrCount(); ++xmtrIndex)
            {
               ...
               result.BeginGenericInteraction(&GetEM_Xmtr(xmtrIndex), mTempGeometryPtr, GetEM_Xmtr(xmtrIndex).GetLinkedReceiver());
               result.ComputeUndefinedGeometry();
               result.SetTransmitterBeamPosition();
               if (result.GetTransmitter() != nullptr)
               {
                  result.GetTransmitter()->NotifyListeners(aSimTime, result);    // Notify listeners for possible passive detection
               }
            }
         }
      }
                                                ...
                                                WsfSensorResult& aResult)
{
   if (aResult.BeginTwoWayInteraction(aXmtrPtr, aTargetPtr, GetEM_Rcvr()) == 0)
   {
      ...
      aResult.mRadarSig = WsfRadarSignature::GetValue(aTargetPtr, aXmtrPtr, GetEM_Rcvr(),
                                                      aResult.mTgtToXmtr.mAz, aResult.mTgtToXmtr.mEl,
                                                      aResult.mTgtToRcvr.mAz, aResult.mTgtToRcvr.mEl);
      ...
      if (mPRF_Factor != 1.0)
      {
         ...
         double closingVelocity = UtVec3d::DotProduct(thisVelocityWCS, toTargetWCS) -
                                  UtVec3d::DotProduct(targetVelocityWCS, toTargetWCS);
         double thisSpeed = UtVec3d::Magnitude(thisVelocityWCS);

         if ((closingVelocity < thisSpeed) &&
             (closingVelocity > -thisSpeed))
         {
      ...
      if (aResult.mFailedStatus == 0)
      {
         ...
         aResult.mSignalToNoise = mRcvrPtr->ComputeSignalToNoise(aResult.mRcvdPower,
                                                                 aResult.mClutterPower,
                                                                 aResult.mInterferencePower);
         ...
         if ((aSettings.mLockonTime >= 0.0) &&
             ((aSettings.mLockonTime + mPostLockonAdjustmentDelayTime) <= aSimTime))
         {
         ...
         if (mProbabilityTablePtr)
         {
            ...
            aResult.mPd = mProbabilityTablePtr->ComputeProbabilityOfDetection(aResult.mSignalToNoise / detectionThresholdAdjustment);
         }
...
{
   double p_avg     = mXmtrPtr->GetPower();
   ...
   if (aPrint)
   {
      ...
      out.AddNote() << "Peak Power Output: " << UtMath::LinearToDB(mXmtrPtr->GetPeakPower()) << " dbW (" << mXmtrPtr->GetPeakPower() << " W)";
      ...
      {
         out.AddNote() << "Receiver Noise Power: " << UtMath::LinearToDB(mRcvrPtr->GetNoisePower()) << " dBW (" << mRcvrPtr->GetNoisePower() << " W)";
      }
   }
   ...
   if (aPrint)
   {
      ...
      out.AddNote() << "Minimum Detectable Signal: " << UtMath::LinearToDB(thresholdWatts) << " dBW (" << thresholdWatts << " W)";
      out.AddNote() << "Minimum Detectable S/N: " << UtMath::LinearToDB(detectionThreshold) << " dB (" << detectionThreshold << ")";
   }
   ...
   if (mOneM2DetectRange > 0.0)
   {
      ...
      if (aPrint)
      {
         out.AddNote() << "Receiver Noise: " << UtMath::LinearToDB(rcvrNoise) << " dbW (" << rcvrNoise << " W) (calibrated)";
      }
   }
   else if (mLoopGain > 0.0)
   {
   ...
   if (mXmtrPtr->GetPulseRepetitionFrequency() != 0.0)
   {
      ...
      if (aMode.GetDwellTime() == 0.0)
      {
         if (mAntennaPtr->GetScanMode() == WsfEM_Antenna::cSCAN_EL)
         {
            ...
            timeOnTarget = aMode.GetFrameTime() * mXmtrPtr->GetElevationBeamwidth() / (maxEl - minEl);
         }
         else
         {
            double minAz, maxAz;
            mAntennaPtr->GetAzimuthScanLimits(minAz, maxAz);
            timeOnTarget = aMode.GetFrameTime() * mXmtrPtr->GetAzimuthBeamwidth() / (maxAz - minAz);
         }
      }
      else
      {
                                           ...
                                           bool         aIsMultiBeam)
{
   ...
   if (aCanReceive)
   {
      ...
      {
         ut::log::warning() << "'signal_processor' should not be used with 'one_m2_detect_range', 'range_product', or 'loop_gain'";
      }
...
// =================================================================================================
// Nested class WsfRadarSensor::RadarMode.负责波束初始化、深拷贝与资源清理
...
WsfRadarSensor::RadarMode& WsfRadarSensor::RadarMode::operator=(const RadarMode& aRhs)
{
   if (this != &aRhs)
   {
      ...
      mOverrideMeasurementWithTruth = aRhs.mOverrideMeasurementWithTruth;
      mCanTransmit = aRhs.mCanTransmit;
...
// =================================================================================================
//virtual初始化每个 RadarBeam 波束，并配置各类探测参数
bool WsfRadarSensor::RadarMode::Initialize(double aSimTime)
{
   ...
   for (unsigned int beamIndex = 0; beamIndex < mBeamList.size(); ++beamIndex)
   {
      ...
      ok &= beam.Initialize(aSimTime, beamIndex, mSensorPtr, this,
                            mCanTransmit, mCanReceive,
                            mSensorPtr->ShowCalibrationData(), isMultiBeam);
   ...
   if (ok)
   {
      ...
      {
         int id = mSensorPtr->GetRandom().Uniform(0, mBeamList[0]->mXmtrPtr->GetAlternateFrequencyCount());
...
bool WsfRadarSensor::RadarMode::ProcessInput(UtInput& aInput)
{
   ...
   if (command == "beam")
   {
      ...
      {
         string msg = "'beam' cannot be used in this context; command " + sLastImplicitBeamCommand + " must be moved inside the 'beam' definition.";
   ...
   else if (command == "compute_measurement_errors")  // DEPRECATE
   {
      bool computeMeasurementErrors;
      aInput.ReadValue(computeMeasurementErrors);
      if (computeMeasurementErrors)
      {
         mErrorModelPtr = ut::make_unique<wsf::RadarSensorErrorModel>();
      }
      else
      {
   ...
   else if (aResult.mFailedStatus == 0)
   {
      ...
      if (mBeamList.size() > 1)
      {
         ...
         for (unsigned int beamIndex = 1; beamIndex < mBeamList.size(); ++beamIndex)
         {
            ...
            {
               tempResult.GetTransmitter()->NotifyListeners(aSimTime, tempResult);     // Notify listeners for possible passive detection
            }
            if (GetSensor()->DebugEnabled())
            {
...
// =================================================================================================
//virtual安排频率切换事件
void WsfRadarSensor::RadarMode::ScheduleAltFreqChange(double aSimTime,
                                                      int    aAltFreqId /*=-1*/)
{
...
//通知监听器
//记录切换时间
//让当前模式所有波束切换到备用频率（支持捷变）
void WsfRadarSensor::RadarMode::SelectAlternateFrequency(double aSimTime,
                                                         int    aAltFreqId /*=-1*/)
{
...
// =================================================================================================
//virtual激活或关闭波束内的发射器/接收器，更新传感器列表
...
// =================================================================================================
// This overrides the version in WsfSensorMode. It implements the beam specific errors.
//protected virtual
//计算并应用测量误差（方位、俯仰、距离等）

void WsfRadarSensor::RadarMode::ApplyMeasurementErrors(WsfSensorResult& aResult)
{
   double azError        = 0.0;
   double elError        = 0.0;
   double rangeError     = 0.0;
   double rangeRateError = 0.0;

   // Compute the measurement errors using the generic technique.
   WsfSensorMode::ComputeMeasurementErrors(aResult, azError, elError, rangeError, rangeRateError);

   if (mOverrideMeasurementWithTruth)
   {
      // Override the measurement error with zero error (truth), but report the measurement error sigmas.
      // This is useful for debugging estimators (trackers) which use these errors to estimate the target position.
      azError        = 0.0;
      elError        = 0.0;
      rangeError     = 0.0;
      rangeRateError = 0.0;
   ...
   measurement.SetRange(aResult.mRcvrToTgt.mRange  + rangeError);
   measurement.SetRangeValid(true);
   measurement.SetSensorAzimuth(aResult.mRcvrToTgt.mTrueAz + azError);
   measurement.SetSensorAzimuthValid(true);
   measurement.SetSensorElevation(aResult.mRcvrToTgt.mTrueEl + elError);
   ...
   aResult.GetReceiver()->GetAntenna()->GetLocationWCS(rcvrToTgtLocWCS,
                                                       reportedLocWCS);
   measurement.SetLocationWCS(reportedLocWCS);

   if (measurement.RangeRateValid())
   {
      measurement.SetRangeRate(measurement.GetRangeRate() + rangeRateError);
   }
}
