...
class WSF_EXPORT WsfEM_Rcvr : public WsfEM_XmtrRcvr
{
   ...
   private:
       // 接收器基础参数
       //RcvrFunction mFunction;          // 接收器功能类型(通信/传感器/干扰)
       //double mFrequency;               // 工作频率(Hz)
       //double mBandwidth;               // 带宽(Hz)
       //double mInstantaneousBandwidth;  // 瞬时带宽(Hz)
       //double mNoisePower;              // 噪声功率(W)
       //double mNoiseFigure;             // 噪声系数(绝对比值)
       //double mDetectionThreshold;      // 检测阈值(3dB above noise)
