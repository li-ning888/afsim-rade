...
// =================================================================================================
//virtual
//•	功能 : 计算传播因子，表示信号在传播过程中的衰减或增益。
//•	输入 :
//•	aInteraction : 包含传播路径的交互信息。
//•	aEnvironment : 包含环境参数（如地形、天气等）。
//•	输出 : 返回传播因子（通常是一个衰减值）。
double WsfEM_FastMultipath::ComputePropagationFactor(WsfEM_Interaction& aInteraction,
                                                     WsfEnvironment&    aEnvironment)
{
   ...
   if ((rcvrPtr != nullptr) &&
       (rcvrPtr->GetFunction() != WsfEM_Rcvr::cRF_PASSIVE_SENSOR))
   {
   ...
   double localEarthRadius = UtVec3d::Magnitude(aInteraction.mRcvrLoc.mLocWCS) -
                             aInteraction.mRcvrLoc.mAlt;
   ...
   ComputeReflectionCoefficient(grazingAngle,
                                epsilon,
...
//virtual
//•	功能 : 计算反射增益，表示信号在反射过程中的能量变化。
//•	输入 :
//•	aXmtrRcvrPtr : 发射器 / 接收器对象。
//•	aBeamData : 波束数据。
//•	aRelTgtLoc : 目标的相对位置。
//•	aDepressionAngle : 下倾角。
//•	aFrequency : 信号频率。
//•	aPolarization : 信号的极化类型。
//•	输出 : 返回反射增益值。
...
// =================================================================================================
//virtual
//•	功能 : 处理用户输入，支持动态配置模型参数。
//•	用途 : 允许用户通过输入文件或命令行修改模型行为。
bool WsfEM_FastMultipath::ProcessInput(UtInput& aInput)
{
   ...
   else if ((command == "surface_roughness") ||
            (command == "stddev_surface_height"))
   {
                                                       ...
                                                       double&                     aPhaseShift)
{
   ...
   if (aPolarization == WsfEM_Types::cPOL_VERTICAL)
   {
      ...
      gamma = std::complex<double>(aEpsilon * sinalp - csrec2) /
              std::complex<double>(aEpsilon * sinalp + csrec2);
   }
   else
   {
      // If the antenna is not vertically polarized then it is assumed to be horizontally polarized.

      gamma = std::complex<double>(sinalp - csrec2) /
              std::complex<double>(sinalp + csrec2);
   }
...
//•	aTgtElevation : 目标高度。
//•	输出 :
//•	aAntToRefSlantRange : 天线到反射点的斜距。
//•	aRefToTgtSlantRange : 反射点到目标的斜距。
//•	aDepressionAngle : 下倾角。
//•	aGrazingAngle : 入射角。
//•	aPathLengthDifference : 路径长度差。
...
{
   static const double frqghz[5] =
   {
      0.3, 3.0, 8.0, 14.0, 24.0
   };

   static const double cmoist[4] =
   {
      0.003, 0.100, 0.200, 0.300
   };
   ...
   double eps1l = ((1.0 - frqfrc) * epsReal[ifreql][iwater]) +
                  (frqfrc * epsReal[ifreql + 1][iwater]);
   double eps1r = ((1.0 - frqfrc) * epsReal[ifreql][iwater + 1]) +
                  (frqfrc * epsReal[ifreql + 1][iwater + 1]);
   ...
   double eps2l = ((1.0 - frqfrc) * epsImag[ifreql][iwater]) +
                  (frqfrc * epsImag[ifreql + 1][iwater]);
   double eps2r = ((1.0 - frqfrc) * epsImag[ifreql][iwater + 1]) +
                  (frqfrc * epsImag[ifreql + 1][iwater + 1]);
