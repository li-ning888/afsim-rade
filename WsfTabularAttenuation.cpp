...
namespace
{
   ...
   bool VariableExists(const std::vector<std::string>& aNames,
...
WsfEM_Attenuation* WsfTabularAttenuation::ObjectFactory(const std::string& aTypeName)
{
   ...
   if ((aTypeName == "WSF_TABULAR_ATTENUATION") ||
       (aTypeName == "tabular"))
   {
...
bool WsfTabularAttenuation::Initialize(WsfEM_XmtrRcvr* aXmtrRcvrPtr)
{
   ...
   if (mAttenuationPtr != nullptr)
   {
      ...
      else if (VariableExists(ivNames, "altitude") &&
               VariableExists(ivNames, "elevation_angle") &&
               VariableExists(ivNames, "slant_range") &&
               (! VariableExists(ivNames, "altitude_1")) &&
               (! VariableExists(ivNames, "altitude_2")) &&
               (! VariableExists(ivNames, "ground_range")))
      {
         mNeedSlantRange = true;
      }
      else if (VariableExists(ivNames, "altitude_1") &&
               VariableExists(ivNames, "altitude_2") &&
               VariableExists(ivNames, "ground_range") &&
               (! VariableExists(ivNames, "altitude")) &&
               (! VariableExists(ivNames, "elevation_angle")) &&
               (! VariableExists(ivNames, "slant_range")))
      {
...
//表格数据处理
  // 处理输入命令：
    // "attenuation" - 加载衰减表
    // "sort_end_points" - 设置端点排序
    // "two_way_attenuation" - 设置是否为双向衰减
    // "query" - 处理查询命令
    // "spectral_data_conversion" - 光谱数据转换
                                                       ...
                                                       WsfEM_Interaction::Geometry aGeometry)
{
   ...
   if (mNeedSlantRange)
   {
      GetRangeElevationAltitude(aInteraction, aGeometry,
                                lookupArgs[cIV_SLANT_RANGE], lookupArgs[cIV_ELEVATION_ANGLE], lookupArgs[cIV_ALTITUDE]);
   }
   else if (mNeedGroundRange)
   {
      GetAltitudesAndGroundRange(aInteraction, aGeometry,
                                 lookupArgs[cIV_ALTITUDE_1], lookupArgs[cIV_ALTITUDE_2], lookupArgs[cIV_GROUND_RANGE]);
   }
   ...
   return (std::min(atten * mAdjustmentFactor, 1.0));
}
...
void WsfTabularAttenuation::SpectralDataConversion(UtInput& aInput)
{
   ...
   while (inputBlock.ReadCommand(command))
   {
      ...
      else if (command == "sensor_response_curve")
      {
         sensorResponseCurve.ProcessInput(input,
                                          UtInput::cLENGTH, "", UtTable::ValueGT(0.0),
                                          UtInput::cNON_DIMENSIONAL, "", UtTable::ValueGE_LE(0.0, 1.0));
   ...
   while (true)
   {
      ...
      else
      {
         ...
         else if ((sttAltitude    != tbrAltitude)  ||
                  (sttElevation   != tbrElevation) ||
                  (sttRange       != tbrRange)     ||
   ...
   for (size_t iAlt = 0; iAlt < altitudeValues.size(); ++iAlt)
   {
      for (size_t iEl = 0; iEl < elevationValues.size(); ++iEl)
      {
         outputFile << "\n      # Altitude=" << altitudeValues[iAlt] << " meters"
                    << ", Elevation angle=" << elevationValues[iEl] << " degrees";
         for (size_t i = 0; i < rangeValues.size(); ++i)
         {
            if ((i % 7) == 0)
            {
...
{
   if ((! getline(aFile, aHeaders[0])) ||
       (! getline(aFile, aHeaders[1])) ||
       (! getline(aFile, aHeaders[2])))
   {
   ...
   while (getline(aFile, line))
   {
      ...
      else if (ch == '%')
      {
         if (line[1] == '{')
         {
            ...
            if ((aAltitude < 0.0)    ||
                (aElevation < -90.0) || (aElevation > 90.0) ||
                (aSlantRange < 0.0))
            {
