...
class WSF_EXPORT RadarSensorErrorModel : public WsfSensorErrorModel
{
   public:

      RadarSensorErrorModel() = default;
      ~RadarSensorErrorModel() override = default;

      static WsfSensorErrorModel* ObjectFactory(const std::string& aTypeName);

      WsfSensorErrorModel* Clone() const override;
      bool Initialize(WsfSensorMode* aSensorModePtr) override;

      void GetSphericalErrorStdDev(ut::Random&      aRandom,
                                   WsfSensorResult& aResult,
                                   Sigmas&          aSigmas,
                                   Errors&          aErrors) override;
   protected:

      RadarSensorErrorModel(const RadarSensorErrorModel& aSrc) = default;
};
