...
template <> struct WsfComponentRole<WsfEM_Rcvr> : std::integral_constant<int, cWSF_COMPONENT_EM_RCVR> {};
...
WsfEM_Rcvr::WsfEM_Rcvr(RcvrFunction   aFunction,
   ...
   : WsfEM_XmtrRcvr(aAntennaPtr),
     ...
     mDetectionThreshold(pow(10.0, 3.0 / 10.0)),    // 3 dB above noise level
...
bool WsfEM_Rcvr::CanInteractWith(WsfEM_Xmtr* aXmtrPtr)
{
   ...
   if ((xmtrFreqLo > rcvrFreqHi) ||
       (xmtrFreqHi < rcvrFreqLo))
   {
...
//virtual
// 计算接收功率(考虑天线增益/极化效应)
double WsfEM_Rcvr::ComputeReceivedPower(double aSourceAz,             // 信号源方位角(弧度)
    double aSourceEl,             // 信号源仰角(弧度) 
    double aEBS_Az,               // 波束方位角(弧度)
    double aEBS_El,               // 波束仰角(弧度)
    double aReceivedPowerDensity, // 接收功率密度(W/m²)
    WsfEM_Types::Polarization aPolarization, // 信号极化方式
    double aFrequency,            // 信号频率(Hz)
    double& aAntennaGain          // 输出天线增益)
)
{
...
//virtual
// 计算信噪比(SNR)
double WsfEM_Rcvr::ComputeSignalToNoise(double aReceivedSignalPower,    // 接收信号功率
                                        double aClutterPower,       // 杂波功率
                                        double aClutterPower,       // 杂波功率
                                        double aReceivedInterferencePower   // 干扰功率)
{
   double signalToNoise = aReceivedSignalPower / (aClutterPower + aReceivedInterferencePower + (mNoisePower * mNoiseMultiplier));
...
bool WsfEM_Rcvr::ProcessInput(UtInput& aInput)
{
   ...
   else if ((command == "instantaneous_bandwidth") ||
            (command == "analysis_bandwidth"))
   {
                                      ...
                                      double aSignalBW) const
{
   ...
   if (rcvrBW == 0.0)
   {
      if ((signalFreqLo <= mFrequency) &&
          (signalFreqHi >= mFrequency))
      {
         rcvrBW = aSignalBW;
      }
      else
   ...
   if ((signalFreqLo > rcvrFreqHi) ||
       (signalFreqHi < rcvrFreqLo))
   {
      effBWRatio = 0.0;
      return effBWRatio;
   }
...
bool WsfEM_Rcvr::AddInteractor(WsfEM_Xmtr* aXmtrPtr)
{
   ...
   switch (aXmtrPtr->GetFunction())
   {
      ...
      {
         if (std::find(mInterferenceInteractors.begin(), mInterferenceInteractors.end(), aXmtrPtr) == mInterferenceInteractors.end())
         {
...
bool WsfEM_Rcvr::RemoveInteractor(WsfEM_Xmtr* aXmtrPtr)
{
   ...
   switch (aXmtrPtr->GetFunction())
   {
      ...
      case WsfEM_Xmtr::cXF_INTERFERER:
      {
         InteractorList::iterator iter = std::find(mInterferenceInteractors.begin(), mInterferenceInteractors.end(), aXmtrPtr);
...
{
   if (CanInteractWith(aXmtrPtr) &&
       aXmtrPtr->AllowInteractionWith(this))
   {
      ...
      if (mInstantaneousBandwidth > 0.0)
      {
         ...
         else
         {
            ...
            if ((mAntennaPtr != nullptr) &&
                (mAntennaPtr->GetArticulatedPart() != nullptr))
            {
               ...
               {
                  elevation = 0.5 * (mAntennaPtr->GetMinimumElevationScanLimit() +
                                     mAntennaPtr->GetMaximumElevationScanLimit());
               }
            }
            ...
            double systemTemperature = ComputeSystemNoiseTemperature(elevation, antennaOhmicLoss, receiveLineLoss,
                                                                     noiseFigure, mFrequency);
            // N = k * Ts * B;
...
//static
// 计算系统噪声温度(基于Blake雷达方程)
double WsfEM_Rcvr::ComputeSystemNoiseTemperature(double aElevation,              // 仰角(弧度)
    double aAntennaOhmicLossFactor, // 天线欧姆损耗
    double aReceiveLineLossFactor,  // 接收线路损耗
    double aNoiseFigure,            // 噪声系数
...
)
{
   ...
   static const double angtab[7] =
   {
      0.0000, 0.0175, 0.0349, 0.0873, 0.1745, 0.5236, 1.5708
   };

   // Frequencies are in MHz
   static const double frq[26] =
   {
         0.0,     10.0,   100.0,   200.0,   300.0,
       400.0,    500.0,   600.0,   700.0,   800.0,
       900.0,   1000.0,  2000.0,  3000.0,  4000.0,
      5000.0,   6000.0,  7000.0,  8000.0,  9000.0,
      10000.0, 12000.0, 16000.0, 20000.0, 22000.0,
      30000.0
   };
   ...
   double tanllg = (frqfac * log10(tantab[ifp1][iang])) +
                   ((1.0 - frqfac) * log10(tantab[ifreq][iang]));

   double tanrlg = (frqfac * log10(tantab[ifp1][iap1])) +
                   ((1.0 - frqfac) * log10(tantab[ifreq][iap1]));
